...
class PokemonBattleEnv(gym.Env):
    ...
    def _apply_effect(self, attacker, defender, effect, log):
        ...
        status_map = {"burn":"burn","paralyze":"paralysis","freeze":"freeze",
                      "sleep":"sleep","poison":"poison","toxic":"poison"}
        if effect in status_map:
            status = status_map[effect]
            ...
            if not getattr(defender, "confused", False):
                defender.confused = True
                defender.confusionTurns = random.randint(2, 5)
                log.append(f"{defender.name}이(가) 혼란에 빠졌다!")
        elif effect == "heal_half":
            heal = max(1, attacker.max_hp // 2)
            attacker.current_hp = min(attacker.max_hp, attacker.current_hp + heal)
            log.append(f"{attacker.name}이(가) HP {heal} 회복!")
        elif effect.startswith("boost_") or effect.startswith("drop_"):
            # 파싱: boost_attack_2 → stat="attack", delta=+2
            #        drop_sp_def_1  → stat="sp_defense", delta=-1
            #        drop_sp_def    → stat="sp_defense", delta=-1
            is_boost = effect.startswith("boost_")
            rest = effect[6:] if is_boost else effect[5:]  # "attack_2" or "sp_def_1" or "sp_def"
            ...
            # 실제 속성 존재 확인
            attr = f"rank_{stat}"
            if not hasattr(attacker, attr):
                return reward  # 알 수 없는 스탯 → 조용히 스킵

            delta = delta_abs if is_boost else -delta_abs
            target = attacker if is_boost else defender
            result = target.change_rank(stat, delta)
            direction = "올랐다" if delta > 0 else "떨어졌다"
            log.append(f"{target.name}의 {stat}이(가) {direction}!")
            reward += 0.2 if is_boost else 0.0
        return reward
